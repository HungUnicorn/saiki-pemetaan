    ...
    if only_check():
        ...
        if request.method == 'POST':
            if mform.validate() is False:
                flash('All fields are required and without whitespace', 'critical')
    ...
    if only_check():
        if request.method == 'POST':
            ...
            if template_form.validate() is False:
                flash('Please check that all the fields are valid and template name contains no white space.',
                      'critical')
