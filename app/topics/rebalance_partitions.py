# copied from https://raw.githubusercontent.com/zalando/saiki-buku/
#     bb74ed278b5fdda792fa034b30de8d8966bdcf6e/src/rebalance_partitions.py
import json
import logging
import os
...
def readout_brokerids(zk):
    try:
        ...
        logging.error("there are no brokers registrated in this " +
                      "zookeeper cluster, therefore exiting")
        exit(1)
...
def readout_topics(zk):
    try:
        ...
        logging.info("there are no topics registrated in " +
...
def readout_partitions(zk, topic):
    try:
        ...
        logging.info("there are no partitions for topic %s in " +
                     "this zookeeper cluster", topic)
    ...
    for topic in zk_dict['topics']:
        ...
        for partition, brokers in topic['partitions'].items():
            ...
            for part_broker_id in brokers:
                logging.debug("checking if this broker is still existing: %s",
                              part_broker_id)
...
def get_own_ip(region):
    if region:
        import requests
        return requests.get('http://169.254.169.254/latest/dynamic/' +
                            'instance-identity/document').json()['privateIp']
    else:
        return '127.0.0.1'
...
def generate_json(zk_dict, topics_to_reassign="all", target_brokers="all"):
    ...
    if set(zk_dict['broker']) - set(target_brokers) != set() and \
            target_brokers != "all":
    ...
    if len(topics_to_reassign) > 0:
        logging.info("topics_to_reassign found, generating " +
                     "new assignment pattern")
        logging.info("reading out broker id's")

        final_result = {'version': 1, 'partitions': []}
        logging.info("generating now ")
        weights = get_broker_weights(zk_dict,
                                     avail_brokers_init,
                                     ignore_existing)
        for topic, partitions in topics_to_reassign.items():
            replication_factor = sorted(map(len,
                                            tmp_topic_dict[topic].values()))[0]
            ...
            for partition in partitions:
                logging.debug("finding new brokers for " +
                              "topic: %s, partition: %s", topic, partition)
                broker_list = [b for b, w in sorted(weights.items(),
                                                    key=lambda v: v[1]
                                                    )][:replication_factor]
...
def write_json_to_zk(zk, final_result):
    ...
    def reassign_partitions_watcher(event):
        if event.type == EventType.DELETED:
            ...
            if zk.exists('/admin/reassign_partitions',
                         reassign_partitions_watcher) is None:
                watcher_event.set()
    ...
    for step in final_result['partitions']:
        ...
        while not done:
            try:
                zk.create("/admin/reassign_partitions",
                          json.dumps({'version': 1, 'partitions': [step]},
                ...
                watcher_event.clear()
                if zk.exists('/admin/reassign_partitions',
                             reassign_partitions_watcher) is not None:
                    watcher_event.wait(300)
            except NodeExistsError:
                try:
                    watcher_event.clear()
                    check = zk.get("/admin/reassign_partitions",
                                   reassign_partitions_watcher)
                    ...
                    for timeout_count in range(0, 6):
                        ...
                        logging.info("there seems to be a reassigning " +
                                     "already taking place: %s",
        ...
        if done is False:
            logging.warning("Reassignment was not successfull due to " +
                            "timeout issues of the previous reassignment")
            break
...
def get_zk_dict(zk):
    ...
    for topic in readout_topics(zk):
        result['topics'].append(
            {'name': topic,
             'partitions': readout_topic_details(zk, topic)['partitions']}
        )
    return result
...
def run(region):
    ...
    result = generate_json(zk_dict,
                           topics_to_reassign=check_for_broken_partitions(
                               zk_dict)
                           )
    if result != {}:
        logging.info("there are %s partitions to repair",
        ...
        if any(weight == 0 for weight in get_broker_weights(zk_dict).values()):
            logging.info("there are unused brokers, " +
                         "reassigning all topics ...")
            result = generate_json(zk_dict)
            if result != {}:
